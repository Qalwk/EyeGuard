...
describe('сигналы для перерыва', () => {
  ...
  it('объединяет неполные моргания и непрерывную работу в сигнал для перерыва', () => {
  ...
  it('не выдаёт предупреждение в первые 15 минут сессии', () => {
    const metrics = metricsFor([
      { timestampMs: 10_000, durationMs: 120, closureDepth: 0.3 },
      { timestampMs: 25_000, durationMs: 120, closureDepth: 0.3 },
      { timestampMs: 40_000, durationMs: 120, closureDepth: 0.3 },
      { timestampMs: 55_000, durationMs: 120, closureDepth: 0.3 },
    ])
