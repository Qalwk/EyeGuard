...
export function MonitorPage() {
  ...
  const {
    ...
    calibrationMessage, startCalibration, isPaused, setPaused,
  } = useEyeMonitoring()
  ...
  const sessionSetup = useMemo<WorkSessionSetup>(() => ({ mode, plannedDurationMinutes: mode === 'free' ? undefined : focusDurationMinutes, breakDurationMinutes: mode === 'free' ? undefined : breakDurationMinutes, goal: sessionGoal }), [breakDurationMinutes, focusDurationMinutes, mode, sessionGoal])
  ...
  }, [dashboard.activeSessionDurationMs, focusDurationMinutes])
  ...
  }, [breakDurationMinutes, focusDurationMinutes, isMonitoring, isPaused, mode])
  ...
  const handleStop = () => { stopMonitoring(); stopTabShare() }
  ...
  return <AppLayout title="Внимательная рабочая сессия" description="Запуск камеры выполняется через демонстрацию этой вкладки: браузер показывает системную панель безопасности.">
